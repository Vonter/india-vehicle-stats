<script lang="ts">
	...
	$: breadcrumbs = generateBreadcrumbs('state-year', state, undefined, year);
	...
	async function loadData() {
		...
		try {
			const rawData = await loadStateYearData(state, parseInt(year));
			data = rawData;
		} catch (err) {
	...
	$: rtosWithMetricTotals =
		data && selectedMetric && data.rtos
			? data.rtos
					...
					// Filter out RTOs with no data for this year/metric
...
<svelte:head>
	<title>{state} Vehicle Statistics - {year}</title>
</svelte:head>
	...
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<h1 class="mb-2 text-3xl font-bold text-foreground sm:text-4xl">
				{state} Vehicle Statistics - {year}
			</h1>
		</div>

		{#if loading}
			<LoadingSpinner size="large" text="Loading {state} {year} data..." />
		{:else if error}
			...
			{#if flatMetrics.length === 0}
				<div class="rounded-lg border border-yellow-200 bg-yellow-50 p-6 text-center">
					...
					<p class="mb-4 text-yellow-700">
						No transaction records found for {state} in {year}.
					</p>
					<div class="space-x-4">
						<a
							href={`/${state}${buildQueryString()}`}
						...
						<div class="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
							{#each displayedRTOs as rto}
								<StatsCard
									...
									href={`/${state}/${rto.RTO}/${year}${buildQueryString()}`}
									compact={false}
						...
						>
							{#each displayedMonths as month}
								<StatsCard
									...
									href={`/${state}/ALL/${year}/${month}${buildQueryString()}`}
									compact={true}
									metric={selectedMetric}
								/>
							{/each}
